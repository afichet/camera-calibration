...
int main(int argc, const char* argv[]) 
{
    ...
    read_spd(
        argv[1],
        &wavelengths_illu_raw, &values_illu_raw, &size_illu_raw 
    );
    ...
    read_cmfs(
        ...
        &values_cmfs_x_raw, &values_cmfs_y_raw, &values_cmfs_z_raw,
        &size_cmfs_raw
    );
    ...
    spectrum_oversample(
        wavelengths_cmfs_raw, values_cmfs_x_raw, size_cmfs_raw,
        &values_cmfs_x, &size_cmfs
    );

    spectrum_oversample(
        wavelengths_cmfs_raw, values_cmfs_y_raw, size_cmfs_raw,
        &values_cmfs_y, &size_cmfs
    );

    spectrum_oversample(
        wavelengths_cmfs_raw, values_cmfs_z_raw, size_cmfs_raw,
        &values_cmfs_z, &size_cmfs
    );

    float macbeth_patches_xyz[24 * 3] = {0};
    get_references_patches_xyz(
        values_cmfs_x, values_cmfs_y, values_cmfs_z,
        first_wavelength_cmfs, size_cmfs,
        values_illu, first_wavelength_illu, size_illu, macbeth_patches_xyz);
